...
public class ConsensusCmd extends CommandLine{
    ...
    public static void main(String[] args) {
        ...
        try {
			...
			seq=ErrorCorrection.consensusSequence(seqs, tmpDir+File.separator+"out", aligner);
			seq.setName("Consensus");
			seq.setDesc("length="+seq.length());
			SequenceOutputStream out = new SequenceOutputStream(new FileOutputStream(output,false));
			seq.writeFasta(out);
			out.close();
        } catch (Exception e1) {
