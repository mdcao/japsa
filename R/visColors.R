...
dendro =try( .plotHeatmap(matrices[[2]], todo = rev(todo1)[1], addspace=T, Colv = F,grps = grps[indsj], hm2=hm2, margins = c(10,15)))
...
pdf(paste(resdir1j,"heatmap_cumul.pdf",sep="/"), height=height, width = width)
 try(.plotHeatmap(matrices[[1]], todo = rev(todo1)[1], addspace=T,Colv=Colv, grps = grps[indsj], hm2=hm2,margins = c(10,15)))
dev.off()



 pdf(paste(resdir1j,"heatmap_sep.pdf",sep="/"), height = height, width =width)
 try(.plotHeatmap(matrices[[1]], todo = todo, Colv = Colv,grps = grps[indsj], hm2=hm2,margins = c(10,15)))
dev.off()
...
ggps = .plotBarchart(matrices[[1]], todo = todo, show.legend=T, legsize=20, colInd=col_inds)
for(i in 1:length(ggps)) print(ggps[[i]])
dev.off()
 pdf(paste(resdir1j,"barcharts_all.pdf",sep="/"), width=width)
ggps1 = .plotBarchart(matrices[[2]], todo = rev(todo1)[1], show.legend=T, legsize=10, colInd=col_inds)
